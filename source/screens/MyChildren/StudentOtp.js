...
class StudentOtp extends React.Component {
  constructor(props) {
    super(props);
 ...
 submitotp = async () => {
  ...
  if (this.state.checkNetwork === true) {
    if (this.state.pin1 !== '' && this.state.pin2 !== '' && this.state.pin3 !== '' && this.state.pin4 !== '' && this.state.pin5 !== '') {
      ...
      APICaller("verifyOtp_link_user",body,'POST',token).then( (response) => {
       
        console.log("response from parent otp line 110",response.data,)
    ...
    console.log(error,"line 146 student otp");
    alert(error.message);
    this.setState({ loading: false });
  });
    } else {
  ...
  render() {
    ...
    return (
      <KeyboardAvoidingView
        ...
        <View style={{flexDirection: 'row',marginTop:hp("2%"),fontWeight:"600",fontSize:15}}>
          <Text style={{marginLeft:wp("4%")}}>We have sent OTP to student <Text style={{fontWeight: 'bold'}}>Contact No</Text></Text>
...
export default connect()(StudentOtp)